...
- ~~Cross-platform installer scripts for Windows, macOS, and Linux that add launchers to PATH, verify Node and Claude Code, and explain PowerShell Core requirements.~~ ✅ Done
- ~~CI that runs syntax checks for PowerShell, Node proxies, Python tests, and markdown links on every pull request.~~ ✅ Done
- ~~Web management dashboard with visual profile, key, provider, and health management.~~ ✅ Done
