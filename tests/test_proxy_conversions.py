...
def start_proxy(script_name: str, port: int, extra_env: Optional[Dict[str, str]] = None) -> subprocess.Popen:
...
def test_proxy_source_contracts() -> None:
    ...
    proxy_utils = (PROXY_DIR / "proxy-utils.js").read_text(encoding="utf-8")
    for marker in (
        "content_block_delta",
        "emitContentBlocks",
        "sendSse",
        ...
        require(marker in proxy_utils, f"proxy-utils.js missing conversion marker: {marker}")
    ...
    for marker in (
        ...
        'ProxyScript = \'Join-Path $PSScriptRoot "..\\proxy\\ollama-cloud-proxy.js"\'',
        'Id = "freetheai"',
    ...
    ollama = (PROXY_DIR / "ollama-cloud-proxy.js").read_text(encoding="utf-8")
    for marker in (
        ...
        require(marker in ollama, f"Ollama Cloud proxy missing marker: {marker}")
