...
Repository: [Shashank-1K/cc-manager](https://github.com/Shashank-1K/cc-manager)

## Highlights

- Profile and model switching through `cc-switch`, `ccs`, and `cc-manage`.
- Provider-first setup, including Anthropic, Gemini, Groq, Mistral, Mistral Vibe, Codestral, OpenCode Nemotron, NVIDIA NIM, Hugging Face, OpenRouter, DeepSeek, Fireworks, Together, xAI, Ollama Cloud, and custom OpenAI-compatible endpoints.
...
- Dedicated provider wrappers for Mistral, Mistral Vibe, Codestral, OpenCode Nemotron, NVIDIA NIM, Hugging Face, Gemini, and OpenRouter normalization.
...
- **Web dashboard** (`cc-dashboard`) for visual profile/key/provider management at `http://127.0.0.1:18200`.
- Comprehensive documentation: [Setup & Usage Guide](./docs/SETUP_AND_USAGE_GUIDE.md) | [Provider Guide](./docs/PROVIDER_GUIDE.md)
...
`cc-manage` also repairs Claude Code's persistent `~/.claude/settings.json` when you run `cc-manage doctor` or launch through `ccs`. It removes cc-manage-managed Anthropic auth/base/model overrides from that file so Claude Code does not see both `ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_API_KEY`. Other settings, such as unrelated `env` entries, are preserved. You can run the repair directly with:
...
5. Launch Claude Code through `ccs` or switch profiles through `cc-switch`.

Mistral Vibe, Codestral, and OpenCode Nemotron are first-class provider choices in the provider picker. They use local proxy wrappers with provider-specific defaults so Claude Code can interact with their chat endpoints through the expected Anthropic-compatible message structure.
...
| OpenCode Nemotron | Proxy | OpenCode `/zen/v1/chat/completions` with tool-call conversion, Claude-only fields stripped, and Anthropic-compatible response shaping. |
...
cc-switch 6 1
ccs
cc-dashboard          # Launch web dashboard at http://127.0.0.1:18200
```
...
cc-manage add  # select OpenCode Nemotron for https://opencode.ai/zen/v1/chat/completions through the local proxy
```
...
cc-manager includes a local web dashboard for visual management of profiles, keys, providers, and system health.

```powershell
cc-dashboard                    # Start on default port 18200
cc-dashboard --port 19000       # Start on custom port
```

Open `http://127.0.0.1:18200` in your browser. The dashboard provides:

| Section | Features |
|---------|----------|
| **Dashboard** | Active profile, profile/key/provider counts, proxy status grid, system info |
| **Profiles** | List, create, edit, delete profiles with provider/model/key assignment |
| **API Keys** | List (redacted), add, edit, delete keys with usage tracking |
| **Providers** | View all 16 providers with modes, ports, and auth info |
| **Proxy Health** | Live port status for all proxy services |
| **Models** | Per-provider model lists with clipboard copy |
| **Doctor** | System health checks (Node.js, Claude Code, keys, proxies) |
| **Settings** | Theme toggle, settings.json repair |

The dashboard binds to `127.0.0.1` only (not network-accessible), uses zero npm dependencies, and reads/writes the same data files as the CLI — fully interchangeable.
