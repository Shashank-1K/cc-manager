...
const AVAILABLE_MODELS = (process.env.CC_MODELS || '')
  ...
  .map(s => s.trim())
  .filter(Boolean);
if (!AVAILABLE_MODELS.length) AVAILABLE_MODELS.push(...FALLBACK_MODELS);
...
const MAX_REQUEST_BYTES = parsePositiveInt(process.env.CC_MAX_REQUEST_BYTES, 0);
...
function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      if (tooLarge) return;
      bytes += chunk.length;
      if (MAX_REQUEST_BYTES && bytes > MAX_REQUEST_BYTES) {
        const mb = (MAX_REQUEST_BYTES / (1024 * 1024)).toFixed(0);
        tooLarge = new Error(`Request too large for Gemini. Maximum is ${mb}MB.`);
        tooLarge.status = 413;
        body = '';
        return;
      }
    ...
    req.on('error', err => reject(tooLarge || err));
  });
}
...
function requestGoogle(path, postData, apiKey) {
  return new Promise((resolve, reject) => {
...
function forwardStream(apiKey, anthropicReq, res) {
  ...
  const endpoint = `/v1beta/models/${googleModel}:generateContent`;
  const postData = JSON.stringify(buildGoogleBody(anthropicReq));
  debugLog(endpoint, postData);

  requestGoogle(endpoint, postData, apiKey).then(result => {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && (requestPath === '/health' || requestPath === '/')) {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, model: AVAILABLE_MODELS[0] }));
  }
  ...
  try {
    const result = await requestGoogle(endpoint, postData, apiKey);
    if (result.status >= 200 && result.status < 300) {
...
const { gracefulShutdown: gracefulShutdownShared } = require('./proxy-utils');
process.on('SIGTERM', () => gracefulShutdownShared(server));
process.on('SIGINT', () => gracefulShutdownShared(server));
