...
$script:_envCache = $null
$script:_envCachePath = $null
...
function Import-ClaudeDotEnv {
    ...
    if ($script:_envCache -and $script:_envCachePath -eq $Path -and $script:_envCache['_lastWrite'] -eq $lastWrite) {
        return $script:_envCache
    }
    ...
    $values['_lastWrite'] = $lastWrite
    $script:_envCache = $values
    $script:_envCachePath = $Path
...
function Get-ProfileProviderGuess {
    ...
    if ($haystack -match "openai") { return "openai-compatible" }
    if ($haystack -match "freetheai|free.ai|free-ai") { return "freetheai" }
    return "anthropic"
}
...
function Get-ProfileModeGuess {
    ...
    if ($ProxyScript -match "ollama") { return "ollama-cloud-proxy" }
    ...
    switch ($Provider) {
        ...
        "ollama-cloud" { "ollama-cloud-proxy" }
