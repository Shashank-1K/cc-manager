...
REPO="${CC_MANAGE_REPO:-Shashank-1K/cc-manager}"
...
chmod +x "$INSTALL_DIR/ccs" "$INSTALL_DIR/cc-switch" "$INSTALL_DIR/cc-status" "$INSTALL_DIR/cc-manage" "$INSTALL_DIR/claude" 2>/dev/null || true
add_path_marker
...
if ! command -v claude >/dev/null 2>&1 && [ ! -x "$HOME/.local/bin/claude" ]; then
  echo "Warning: Claude Code was not found. Install Claude Code before launching ccs." >&2
fi
...
Next:
  ...
  cc-switch
  ccs
  cc-dashboard       # Web dashboard at http://127.0.0.1:18200
