...
$Repo = if ($env:CC_MANAGE_REPO) { $env:CC_MANAGE_REPO } else { "Shashank-1K/cc-manager" }
...
try {
    ...
    if (!$claude -and !(Test-Path (Join-Path $HOME ".local\bin\claude.exe"))) {
        Write-Host "Warning: Claude Code was not found. Install Claude Code before launching ccs." -ForegroundColor Yellow
    }
    ...
    Write-Host "  ccs"
    Write-Host "  cc-dashboard       # Web dashboard at http://127.0.0.1:18200"
} finally {
